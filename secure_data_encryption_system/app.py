...
#          time_remaining = user_data.get("lock_until", 0) - time.time()
...
#          st.stop()
#       data_input = st.text_input("Enter text:")
#       if st.button("encrypt"):
#          if data_input:
#            ecrypted = encrypt_text(data_input)
#            db = load_data()
#            db[st.session_state.username]["encryptions"].append(ecrypted)
#            save_data(db)
...
#                 button_key = f"decrypt_{index}"
        
#                 if st.button(f"Decrypt", key=button_key):
#                    try:
#                      decrypted_text = decrypt_text(encryption)
#                      st.session_state[f"decrypted_text_{index}"] = decrypted_text
#                      st.success("Text successfully decrypted!")
#                    except Exception as e:
#                      st.error(f"Error decrypting text: {str(e)}")
        
#               if f"decrypted_text_{index}" in st.session_state:
#                 st.write(f"**Decrypted text**: {st.session_state[f"decrypted_text_{index}"]}")
